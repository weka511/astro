...
def compose(omega=0,I=0,Omega=0):
    ...
    P1 = np.array([[cos_omega, -sin_omega, 0],
    ...
    P3 = np.array([[cos_Omega, -sin_Omega, 0],
                   [sin_Omega,  cos_Omega, 0],
                   [0,          0,         1]])
   
    return np.matmul(P3, np.matmul(P2, P1))
...
def create_orbit(planet,
    ...
    Zs                        = []
    ts                        = []
    Rotation                  = compose(omega = np.radians(varpi - Omega), #MD 2.118
                                         I     = np.radians(I),
...
class TestJupiter(TestCase):
    ...
    def test_compose(self):
        rotation = compose(omega=np.radians(14.7392 -100.535 ),I=np.radians(1.30537),Omega=np.radians(100.535))
