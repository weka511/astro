...
def rotate3D(omega=0,I=0,Omega=0):
    ...
    return np.matmul(create_P1(Omega),
                     np.matmul(create_P2(I), 
                               create_P1(omega)))
...
def get_longitude_of_perihelion(T=0,varpi0=14.75385,varpi_dot=839.93):
    ...
    Parameters:
        ...
        varpi_dot
    '''
    return np.radians(varpi0 + varpi_dot * T / 3600)

def get_true_anomaly(E,e,N=1000,atol=1e-12):
    '''
    ...
    for i in range(N):
        if abs(f) < atol: return f
        correction = (np.cos(f)-cos_f)/np.sin(f)
        f += correction
        if (abs(correction) < atol): return f
        
    raise ValueError(f'Correction {correction} is still greater than {atol} after {N} iterations.')       
...
def get_xy(T=0,lambdaT=0,e=0.0484007,a=5.20332,varpi = 14.7392):
    ...
    Parameters:
        T          Number of Julian centuries np.since start of epoch
        lambdaT    Mean longitude in radians
        e          eccentricity
        a          Semi-major axis
        var        longitude of perihelion in radians
    '''
    M = lambdaT - varpi #MD: paragraph before 2.123
    E = get_eccentric_anomaly(e=e,M=M)
    return a * (np.cos(E) - e),a * np.sqrt(1 - e*e)*np.sin(E)
...
def is_minimum(a,b,c):
    '''
    Verify that value is a minimum
...
class TestJupiter(TestCase):

    def test_rotate3D(self):
        '''
        This test uses Murray & Dermott (2.124)
        '''
        rotation = rotate3D(omega=np.radians(14.7392 -100.535 ),I=np.radians(1.30537),Omega=np.radians(100.535))
    ...
    def test_get_XYZ(self):
        ...
        r = np.sqrt(x*x + y * y)
        f = np.arctan2(y,x)
        X,Y,Z = get_XYZ(omega=np.radians(14.7392 -100.535 ),I=np.radians(1.30537),Omega=np.radians(100.535),r=r,f=f)
    ...
    def test_get_mean_longitude(self):
        self.assertAlmostEqual(np.radians(204.234),get_mean_longitude(T=-0.06266423)%(2*np.pi),places=3)
    ...
    def test_get_longitude_of_perihelion(self):
        self.assertAlmostEqual(np.radians(14.7392),
...
class TestKepler(TestCase):
    ...
    def test_kepler_jupiter(self):
        ...
        '''
        self.assertAlmostEqual(np.radians(189.059),
                               get_eccentric_anomaly(
                                   M=np.radians(189.495),
                                   e=0.0484007),
                               places=4)
