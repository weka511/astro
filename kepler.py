...
class Kepler(Hamiltonian):
    ...
    Attributes:
        ...
        m       mass
        k       G*m1*m2
        xi      Vector which includes constants of motion
    '''
    ...
    def _invert(self,hamiltonian):
        ...
        r = -self.k/self.xi[0]
        L = self.xi[2]
        p_squared = 2*self.m*self.xi[1] - L**2/(r*r)        
    ...
    def hamiltonian(self):
        ...
        return (self.x[2]**2/(2*self.m) + 
               self.x[3]**2/(2*self.m*self.x[0]**2) 
               - self.k/self.x[0])
...
def parse_args():
    ...
    parser.add_argument('-k',type=float,default=1.0,help='G*m1*m2')
